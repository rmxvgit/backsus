import { getDocumentHeaderString, HeaderStrParams } from "./folhaInicial";
...
export function getFinalDocument(params: finalDocParams): string[] {
   // Caminho para o arquivo CSV
    ...
    // Ler o arquivo CSV
    const csv_individualizada = readFileSync(pathToData("calculo_IVR_TUNEP_individualizado.csv"), 'utf-8');
    const csv_mensal = readFileSync(pathToData("calculo_IVR_TUNEP_mensal.csv"), 'utf-8');
    ...
    const procedimento_acumulado = readFileSync(pathToData("resumo_mes.csv"), 'utf-8');
    ...
    return [header /*+ resumoTotal*/ + resumo_anual  + resumoMes + procedimentoAcumulado + mensal + individualizada + endDocument, total];
}
